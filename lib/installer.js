...
function getHookCommands() {
  ...
  if (p === 'win32') {
    ...
    return {
      SessionStart: `powershell -NonInteractive -File "${base}\\pick-voice.ps1"`,
      Notification: `powershell -NonInteractive -File "${base}\\notify.ps1"`,
      Stop: `powershell -NonInteractive -File "${base}\\announce-stop.ps1"`
    };
  }
...
function install() {
  ...
  for (const name of ['pick-voice', 'notify', 'announce-stop']) {
    ...
    if (p !== 'win32') fs.chmodSync(dst, 0o755);
