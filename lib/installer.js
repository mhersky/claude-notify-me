...
function install() {
  ...
  // Pre-create last-activity.txt with a default so the Write tool can Read it first
  const activityFile = path.join(CLAUDE_DIR, 'last-activity.txt');
  if (!fs.existsSync(activityFile)) {
    fs.writeFileSync(activityFile, 'No recent activity', 'utf8');
    console.log('  Created last-activity.txt');
  }
